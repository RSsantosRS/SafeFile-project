...
def ir_para_inicio():
    try:
        ...
        if os.path.exists(caminho_inicio_py):
            janela.destroy() # Destruir a janela atual ANTES de abrir a nova
            ...
            # Considere usar tkinter.messagebox.showerror aqui se a janela não for destruída
...
def ir_para_cadastro():
    ...
    try:
        # Constrói o caminho para cadastro.py no mesmo diretório do script atual
        script_dir = os.path.dirname(__file__) # Diretório do script atual
        caminho_cadastro_py = os.path.join(script_dir, "cadastro.py")

        # Verifica se o arquivo cadastro.py existe antes de tentar executá-lo
...
janela = tk.Tk()
janela.title("SafeFile - Menu Principal") # Alterado para refletir que é um menu
LARGURA_JANELA = 800
ALTURA_JANELA = 600
...
cor_botao_login_bg = "#FFFFFF" # Usado para o botão de Login
cor_botao_login_fg_text = "#5C00A4" # Texto do botão de Login
cor_botao_secundario_fg_text = "white" # Para botões de cadastro e início
cor_fundo_botao_secundario = cor_azul_escuro # Fundo para botões de cadastro e início

# --- Fontes (usando tuplas para compatibilidade com create_text e Button) ---
fonte_titulo_str = ("Arial", 36, "bold")
fonte_botao_principal_str = ("Arial", 18, "bold") # Para o botão Login
fonte_botao_secundario_str = ("Arial", 12) # Para os outros botões
...
ALTURA_BOTAO_PRINCIPAL_PIXELS = 50 # Para o botão Login
ALTURA_BOTAO_SECUNDARIO_PIXELS = 45 # Para os outros botões

# Botão Login
botao_login = tk.Button(
    janela,
    text="Login",
    font=fonte_botao_principal_str, # Fonte principal
...
botao_cadastro = tk.Button(
    ...
    font=fonte_botao_secundario_str, # Fonte secundária
...
botao_inicio = tk.Button(
    janela,
    text="Voltar para o Início", # Texto ajustado
    font=fonte_botao_secundario_str, # Fonte secundária
    fg=cor_botao_secundario_fg_text,
    bg=cor_fundo_botao_secundario,
    activebackground=cor_azul_claro,
...
# --- Layout dos Botões (usando place) ---
RELY_LOGIN = 0.50 # Ajustado para subir um pouco e dar espaço
RELY_CADASTRO = 0.62 # Ajustado
RELY_INICIO = 0.74 # Ajustado
